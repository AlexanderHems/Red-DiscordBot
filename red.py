        ...
        if message.channel.id not in shush_list:
            if message.content == client.user.name.upper() or message.content == client.user.name.upper() + "?":
                ...
                await client.send_message(message.channel,
                                          "{} `Check your DMs for the command list.`".format(message.author.mention))
                ...
                await client.send_message(message.channel,
                                          "{}: ".format(message.author.mention) + "`" + choice(ball) + "`")
                ...
                await client.send_message(message.channel,
                                          "{} `Your id is {}`".format(message.author.mention, message.author.id))
                ...
                await client.send_message(message.channel,
                                          "{} `Check your DMs for " + p + "meme help.`".format(message.author.mention))
                ...
                if checkAuth("Trivia", message, settings):
                    if message.content == p + "trivia stop":
                        if getTriviabyChannel(message.channel):
                            ...
                            await client.send_message(message.channel,
                                                      "`There's no trivia session ongoing in this channel.`")
                        ...
                        await client.send_message(message.channel,
                                                  "`A trivia session is already ongoing in this channel.`")
                else:
...
async def on_message_edit(before, message):
    if message.author.id != client.user.id and settings["FILTER"] and not isMemberAdmin(
            message) and not message.channel.is_private:
...
def loggerSetup():
    ...
    handler.setFormatter(
        logging.Formatter('%(asctime)s:%(levelname)s:%(name)s: %(message)s', datefmt="[%d/%m/%Y %H:%M]"))
...
class Trivia():
    ...
    async def newQuestion(self):
        ...
        while self.status != "correct answer" and abs(self.timer - int(time.perf_counter())) <= settings[
            "TRIVIA_DELAY"]:
    ...
    async def sendTable(self):
        self.scoreList = sorted(self.scoreList.items(), reverse=True,
...
class Playlist():
    def __init__(self,
                 filename=None):  # a playlist with a single song is just there to make !addfavorite work with !youtube command
                ...
                if isPlaylistValid([self.playlist[nextTrack]]):  # Checks if it's a valid youtube link
                    if settings["DOWNLOADMODE"]:
                        ...
                        try:
                            ...
                            musicPlayer = client.voice.create_ffmpeg_player("cache/" + path,
                                                                            options='''-filter:a "volume={}"'''.format(
                                                                                settings["VOLUME"]))
                            musicPlayer.start()
                        except:
                        ...
                        musicPlayer = client.voice.create_ytdl_player(self.playlist[nextTrack],
        ...
        if settings["DOWNLOADMODE"]:
            ...
            if musicPlayer:
                if musicPlayer.is_playing():
                    musicPlayer.stop()
            if v:
                musicPlayer = client.voice.create_ffmpeg_player("cache/" + v,
                                                                options='''-filter:a "volume={}"'''.format(
                                                                    settings["VOLUME"]))
                musicPlayer.start()
        else:
...
async def startPoll(message):
    ...
    if not getPollByChannel(message):
        ...
        if p.valid:
            ...
            await client.send_message(message.channel,
                                      "`" + settings["PREFIX"] + "poll question;option1;option2 (...)`")
    else:
        ...
        if len(msg) > 2:
            ...
            if len(newcmd) > 1 and newcmd.find(" ") == -1:
                ...
                if newcmd not in cmdlist:
                    ...
                    await client.send_message(message.channel, "`This command already exists. Use " + settings[
                        "PREFIX"] + "editcom [command] [text]`")
        ...
        if len(msg) > 2:
            ...
            if message.channel.server.id in commands:
                ...
                if cmd in cmdlist:
                    ...
                    await client.send_message(message.channel, "`That command doesn't exist. Use " + settings[
                        "PREFIX"] + "addcom [command] [text]`")
            else:
                await client.send_message(message.channel,
                                          "`There are no custom commands in this server. Use " + settings[
                                              "PREFIX"] + "addcom [command] [text]`")
...
async def randomchoice(message):
    ...
    if len(msg) == 1:
        await client.send_message(message.channel,
                                  "`" + settings["PREFIX"] + "choose option1 or option2 or option3 (...)`")
    elif len(msg) >= 2:
...
async def imdb(message):  # Method added by BananaWaffles.
    ...
    if apis["MYAPIFILMS_TOKEN"] == "TOKENHERE":
        await client.send_message(message.channel,
                                  "`This command wasn't configured properly. If you're the owner, edit json/apis.json`")
        return False
    if len(msg) > 1:
        if len(msg[1]) > 1 and len([msg[1]]) < 20:
            try:
                msg.remove(msg[0])
                msg = "+".join(msg)
                search = "http://api.myapifilms.com/imdb/title?format=json&title=" + msg + "&token=" + apis[
                    "MYAPIFILMS_TOKEN"]
...
async def memes(message):
    ...
    if len(msg) == 3:
        if len(msg[0]) > 1 and len([msg[1]]) < 20 and len([msg[2]]) < 20:
            try:
                search = "https://api.imgflip.com/caption_image?template_id=" + msg[0] + "&username=" + apis[
                    "IMGFLIP_USERNAME"] + "&password=" + apis["IMGFLIP_PASSWORD"] + "&text0=" + msg[1] + "&text1=" + \
                         msg[2]
                async with aiohttp.get(search) as r:
                    result = await r.json()
                if result["data"] != []:
            ...
            await client.send_message(message.channel,
                                      "`" + settings["PREFIX"] + "meme id;text1;text2   |  " + settings[
                                          "PREFIX"] + "meme help for full list`")
    else:
        await client.send_message(message.channel, "`" + settings["PREFIX"] + "meme id;text1;text2   |  " + settings[
            "PREFIX"] + "meme help for full list`")
    ...
    if len(msg) == 2:
        if msg[1].isdigit():
            ...
            if msg[1] < 99999 and msg[1] > 1:
                await client.send_message(message.channel,
                                          "{} :game_die: `{}` :game_die:".format(message.author.mention,
                                                                                 str(randint(1, msg[1]))))
            else:
                await client.send_message(message.channel,
                                          "{} `A number between 1 and 99999, maybe? :)`".format(message.author.mention))
        else:
...
async def twitchCheck(message):
    ...
    if len(msg) == 2:
        try:
            ...
            if "error" in data:
                await client.send_message(message.channel,
                                          "{} `There is no streamer named {}`".format(message.author.mention, msg[1]))
            elif "stream" in data:
                if data["stream"] != None:
                    await client.send_message(message.channel,
                                              "{} `{} is online!` {}".format(message.author.mention, msg[1],
                                                                             "http://www.twitch.tv/" + msg[1]))
                else:
                    await client.send_message(message.channel,
                                              "{} `{} is offline.`".format(message.author.mention, msg[1]))
            else:
                await client.send_message(message.channel,
                                          "{} `There is no streamer named {}`".format(message.author.mention, msg[1]))
        except:
        ...
        await client.send_message(message.channel,
                                  "{} `".format(message.author.mention) + settings["PREFIX"] + "twitch [name]`")
    ...
    if not client.is_voice_connected():
        if message.author.voice_channel:
            if message.author.voice_channel.permissions_for(message.server.me).connect:
                ...
                await client.send_message(message.channel, "{} `I need permissions to join that channel.`".format(
                    message.author.mention))
                return False
        else:
            await client.send_message(message.channel,
                                      "{} `You need to join a voice channel first.`".format(message.author.mention))
            return False
    return True
...
async def playVideo(message):
    ...
    if await checkVoice(message):
        ...
        stopMusic()
        if settings["DOWNLOADMODE"]:
            toDelete = await client.send_message(message.channel,
                                                 "`I'm in download mode. It might take a bit for me to start. I'll delete this message as soon as I'm ready.`".format(
                                                     id, message.author.name))
        data = {"filename": 'https://www.youtube.com/watch?v=' + id, "type": "singleSong"}
        currentPlaylist = Playlist(data)
        if canDeleteMessages(message):
            await client.send_message(message.channel, "`Playing youtube video {} requested by {}`".format(
                await youtubeparser.getTitle(currentPlaylist.playlist[currentPlaylist.current]), message.author.name))
            await client.delete_message(message)
        if toDelete:
        ...
        if msg != p + "play" or msg != "play ":
            if await checkVoice(message):
                ...
                if dataIO.fileIO("playlists/" + msg + ".txt", "check"):
                    ...
                    if settings["DOWNLOADMODE"]:
                        toDelete = await client.send_message(message.channel,
                                                             "`I'm in download mode. It might take a bit for me to start and switch between tracks. I'll delete this message as soon as the current playlist stops.`".format(
                                                                 id, message.author.name))
                    currentPlaylist = Playlist(data)
                    ...
                    await client.send_message(message.channel,
                                              "{} `That playlist doesn't exist.`".format(message.author.mention))
    else:
        if await checkVoice(message):
            stopMusic()
        ...
        if len(msg) == 2:
            ...
            if localplaylists and ("/" not in msg[1] and "\\" not in msg[1]):
                if msg[1] in localplaylists:
                    ...
                    await client.send_message(message.channel,
                                              "`There is no local playlist called {}. " + p + "local or " + p + "locallist to receive the list.`".format(
                                                  msg[1]))
            else:
...
async def getRandomSteamGame(message):
    ...
    elif "is_free" in storeJson[appid]["data"] & storeJson[appId]["data"]["is_free"]:
        price = "Game is Free 2 Play"
    else :
    ...
    if msg["filename"].endswith(".txt"):
        if not dataIO.fileIO("playlists/" + msg["filename"], "check"):  # returns false if file already exists
            ...
            if isPlaylistValid(data) and isPlaylistNameValid(msg["filename"].replace(".txt", "")):
                ...
                await client.send_message(message.channel,
                                          "`Playlist added. Name: {}`".format(msg["filename"].replace(".txt", "")))
            else:
                await client.send_message(message.channel,
                                          "`Something is wrong with the playlist or its filename. Type " + settings[
                                              "PREFIX"] + "audio help to read how to format it properly.`")
        else:
            await client.send_message(message.channel,
                                      "`A playlist with that name already exists. Change the filename and resubmit it.`")
    ...
    if len(msg) == 3:
        ...
        if isPlaylistNameValid(name) and len(name) < 25 and isPlaylistLinkValid(link):
            ...
            if links:
                ...
                await client.send_message(message.channel,
                                          "`Something went wrong. Either the link was incorrect or I was unable to retrieve the page.`")
        else:
            await client.send_message(message.channel,
                                      "`Something is wrong with the playlist's link or its filename. Remember, the name must be with only numbers, letters and underscores. Link must be this format: https://www.youtube.com/playlist?list=PLe8jmEHFkvsaDOOWcREvkgFoj6MD0pXXX`")
...
async def addToFavorites(message):
    if currentPlaylist:
        ...
        await client.send_message(message.channel,
                                  "{} `This song has been added to your favorites.`".format(message.author.mention))
    else:
    ...
    if currentPlaylist:
        if dataIO.fileIO("favorites/" + message.author.id + ".txt", "check"):
            ...
            if currentPlaylist.playlist[currentPlaylist.current] in data:
                ...
                dataIO.fileIO("favorites/" + message.author.id + ".txt", "save", data)
                await client.send_message(message.channel,
                                          "{} `This song has been removed from your favorites.`".format(
                                              message.author.mention))
            else:
                await client.send_message(message.channel,
                                          "{} `This song isn't in your favorites.`".format(message.author.mention))
        else:
            await client.send_message(message.channel,
                                      "{} `You don't have any favorites yet. Start adding them with " + settings[
                                          "PREFIX"] + "addfavorite`".format(message.author.mention))
    else:
        await client.send_message(message.channel, "{} `No song is being played`".format(message.author.mention))
...
async def playFavorites(message):
    ...
    if await checkVoice(message):
        if dataIO.fileIO("favorites/" + message.author.id + ".txt", "check"):
            ...
            await client.send_message(message.channel,
                                      "{} `You don't have any favorites yet. Start adding them with !addfavorite`".format(
                                          message.author.mention))
    ...
    if len(msg.split(" ")) == 2:
        ...
        try:
            ...
            if vol >= 0 and vol <= 1:
                ...
                await(
                client.send_message(message.channel, "`Volume must be between 0 and 1. Example: " + p + "volume 0.50`"))
        except:
            await(
            client.send_message(message.channel, "`Volume must be between 0 and 1. Example: " + p + "volume 0.15`"))
    else:
...
async def downloadMode(message):
    if isMemberAdmin(message):
        if settings["DOWNLOADMODE"]:
            ...
            await(client.send_message(message.channel,
                                      "`Download mode disabled. This mode is unstable and tracks might interrupt. Also, the volume settings will not have any effect.`"))
        else:
    ...
    if isMemberAdmin(message):
        ...
        if len(msg) >= 2:
            ...
            await client.send_message(message.channel, "`" + settings[
                "PREFIX"] + "addwords [word1] [word2] [phrase/with/many/words] (...)`")
    else:
        await client.send_message(message.channel, "`I don't take orders from you.`")
...
async def removeBadWords(message):
    ...
    if isMemberAdmin(message):
        ...
        if len(msg) >= 2:
            ...
            await client.send_message(message.channel, "`" + settings[
                "PREFIX"] + "removewords [word1] [word2] [phrase/with/many/words](...)`")
    else:
        await client.send_message(message.channel, "`I don't take orders from you.`")
    ...
    if isMemberAdmin(message):
        ...
        if len(msg) == 2:
            ...
            for stream in twitchStreams:
                if stream["NAME"] == msg[1] and message.channel.id not in stream[
                    "CHANNELS"]:  # twitchAlert is already monitoring this streamer but not in this channel
            ...
            await client.send_message(message.channel,
                                      "`I will always send an alert in this channel whenever {}'s stream is online. Use !stoptwitchalert [name] to stop it.`".format(
                                          msg[1]))
        else:
        ...
        if len(msg) == 2:
            for i, stream in enumerate(twitchStreams):
                if stream["NAME"] == msg[1] and message.channel.id in stream["CHANNELS"]:
                    ...
                    dataIO.fileIO("json/twitch.json", "save", twitchStreams)
                    await client.send_message(message.channel,
                                              "`I will stop sending alerts about {}'s stream in this channel.`".format(
                                                  msg[1]))
                    return True
            await client.send_message(message.channel,
                                      "`There's no alert for {}'s stream in this channel.`".format(msg[1]))
        else:
...
async def debug(message):  # If you don't know what this is, *leave it alone*
    if message.author.id == settings["DEBUG_ID"]:  # Never assign DEBUG_ID to someone other than you
        ...
        if len(msg) == 3:
            ...
            try:
                result = str(eval(cmd))
                if settings["PASSWORD"].lower() not in result.lower() and settings[
                    "EMAIL"].lower() not in result.lower():
                    await client.send_message(message.channel, "```" + result + "```")
                else:
                    await client.send_message(message.author,
                                              "`Are you trying to send my credentials in chat? Because that's how you send my credentials in chat.`")
            except Exception as e:
